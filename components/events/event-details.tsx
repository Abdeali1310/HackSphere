...
import { Calendar, Users, ExternalLink, Trophy, CheckCircle, Circle, UserPlus } from "lucide-react"
...
export function EventDetails({ event, currentUserRole, currentUserId, onEdit, onBack }: EventDetailsProps) {
  ...
  const handleUnregister = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to unregister",
        variant: "destructive",
      })
    }
  }
        ...
        <TabsContent value="tracks" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Different categories participants can compete in</CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                ...
                <p className="text-muted-foreground">No tracks defined for this event.</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="prizes" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Rewards for top performing teams</CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                <div className="space-y-3">
                    ...
                    <div key={prize.id} className="border rounded-lg p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {prize.sponsor && (
                            <p className="text-xs text-muted-foreground mt-2">Sponsored by {prize.sponsor}</p>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <p className="text-muted-foreground">No prizes defined for this event.</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card>
            ...
            <CardContent>
              {loading ? (
                    ...
                    <div key={milestone.id} className="border rounded-lg p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground mt-2">Due: {formatDate(milestone.dueDate)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <p className="text-muted-foreground">No milestones defined for this event.</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
